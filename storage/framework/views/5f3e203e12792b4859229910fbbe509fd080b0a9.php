      ...
      <div class="container" data-aos="fade-up">
        ...
        <header class="section-header section-header-new">
          ...
          <?php
            ...
            $artistDetails = DB::select('select * from users where id = ?', [$aid]);

            foreach ($artistDetails as $artist) {
                $name = $artist->name;
              }
        ...
        <div class="row topten-cols">
          ...
          <div class="col-12 profile-cont ali">
            <?php
            ...
            foreach ($artistDetails as $artist) {
                ...
                $totalSongs = DB::select('select * from songs where artist_id = ?', [$id]);
                $nos = count($totalSongs);

                $totalalbums = DB::select('select DISTINCT name from albums where artist_id = ? ORDER BY name', [$id]);
                $albumno = count($totalalbums);

            echo"

            <div class='col-12 col-md-6 image text-center'>
              <img src='../../user_pictures/$picture' alt='' class='img-fluid profile'>
            </div>
            <div class='col-12 col-md-4 captions'>
              <br>
              <span class='profile-caption'>Genre </span><br><span class='profile-detail'>$genre</span>
              <br><br>
              <span class='profile-caption'>Singles </span><br><span class='profile-detail'>$nos</span><br><br>
              <span class='profile-caption'>Albums </span><br><span class='profile-detail'>$albumno</span><br><br>
              <span class='profile-caption'>About </span><br><span class='profile-detail'>$about</span><br><br>
            </div>
        ...
        echo "<span class='col-12 text-white text-center text-uppercase'><br><br>singles<br><br><br></span>";

          $aid = $artistId;

          $artistSongs = DB::select('select * from songs where artist_id = ? ORDER BY downloads DESC', [$aid]);
          ...
          <div class='col-md-4'>
            <div class='topten-col'>
              <div class='cont'>
                ...
                <a class='bg-transparent border-link text-capitalize' name='song' href='/{$id}/{$comb}'>$artist - $title <span class='bi bi-play-circle iconx'></span>
                <br>
                <span class='caption text-capitalize'>$genre</span><br>
                <span class='caption text-capitalize'>Downloaded: $downloads</span>
                </a>
                <br>
              </div>
            </div>
        ...
        echo "<span class='col-12 text-white text-center text-uppercase'><br><br>Albums<br><br><br></span>";

        $albums = DB::select('select DISTINCT name from albums where artist_id = ? ORDER BY name', [$aid]);

        foreach ($albums as $albumName) {

          $OneAlbumName = $albumName->name;

          $albumDetails = DB::select('select * from albums where name = ? LIMIT 1', [$OneAlbumName]);
            ...
            <div class='col-md-4'>
              <div class='topten-col'>
                <div class='cont'>
                  ...
                  <a class='bg-transparent border-link text-capitalize' name='album' href='/album/{$name}/{$comb}'>$artist - $name <span class='bi bi-play-circle iconx'></span>
                  <br>
                  <span class='caption text-capitalize'>$nos Tracks<br>$genre</span><br>
                  </a>
                  <br>
                </div>
              </div>
        ...
        <div class='col-12 text-center'>
          <a onclick='back()' class='col-3 btn btn-sm btn-primary m-2'>Previous</a><br/>
          <a href='/' class='col-3 btn btn-sm btn-primary'>Back To Home</a> <br>
        </div>
      </div>
    </section>
...
<?php /**PATH D:\musicworld\resources\views/artist.blade.php ENDPATH**/ ?>